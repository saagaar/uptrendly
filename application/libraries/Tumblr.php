...
require_once(APPPATH.'/third_party/tumblr/OAuthtumblr.php');
// use tumblr\OAuthExceptiontumblr;
// use tumblr\OAuthSignatureMethod_HMAC_SHA1;
// use Eher\OAuth\consumer;
// use Eher2\OAuth\HmacSha1;
...
class Tumblr{
  ...
  function __construct($params = array())
  {
    ...
    if (count($params) > 0)
    {
      ...
      $this->tumblr_consumer_key = TUMBLR_APP_KEY;
      $this->tumblr_secret_key = TUMBLR_APP_SECRET;
    ...
    // }
      ...
      $this->callback_url = TUMBLR_APP_REDIRECT_URI;
   
    
    // if(!isset($this->auth_callback)) {
      $this->auth_callback = TUMBLR_APP_REDIRECT_URI;
    // }
    ...
    elseif($this->ci->session->userdata('request_token') && $this->ci->session->userdata('request_token_secret'))
    {
      ...
      // echo $this->ci->session->userdata('request_token')
  ...
  function get_access_token($oauth_verifier = FALSE) 
  {
    ...
    if(!(isset($token['oauth_token'] ) || isset($token['oauth_token_secret']) ))
    {
      return array('error_message'=>'Request rejected');
    }
    $oauth=$token['oauth_token'];
    $secret=$token['oauth_token_secret'];
    $this->token = new OAuthConsumer($oauth, $secret);
    return $token;
  }
  /**
  ...
  function handle_auth()
  {
    ...
    // echo $this->callback_url;
      ...
      $request_token = $this->get_request_token(($this->callback_url));
      ...
      if(isset($request_token['oauth_token']) && isset($request_token['oauth_token_secret'])){
          ...
          $this->ci->session->set_userdata('request_token', '');
          $this->ci->session->set_userdata('request_token_secret', '');
      }
      
      if($this->connection->http_code == 200)
      {
       return $url = $this->connection->get_authorize_url($request_token);
        // redirect($url);
      }
      else
      {

...
class Admin extends CI_Controller {
	...
	public function sent(){
   	...
   	public function get_product_by_useremail($email)
   	{
   		$html='';
   		$newemail=urldecode($email);
   		$member=$this->general->get_single_row('members',array('email'=>trim($newemail)));

   		 $user_type=$member->user_type;
   		if($user_type=='3')
   		{
			$product=$this->general->get_product_by_user($member->id);
   			
   		}
   		if($user_type=='4')
   		{
   			$product=$this->general->get_active_bid_by_user($member->id);
   		}
   		...
   		if(is_array($product)&& count($product)>0)
   		{
	   		...
	   		{
	   			$html=$html."<option value=".$eachproduct->product_id.">".$eachproduct->name."</option>";
	   		}
   		}
    ...
    public function compose($id=false){
		$data = array();
		...
		if($id)
		{
			$member=$this->general->get_single_row('members',array('id'=>$id));
			if(count($member)>0 && is_object($member)){
		...
		if($type == 'outbox' OR $type =='inbox')
		{
			...
			if ($this->input->server('REQUEST_METHOD') === 'POST')
			{
				...
				switch($this->input->post('conversation_actions')){
					case 'make_read': $trans_stat = $this->admin_communication_model->change_admin_message_status('1',$type); break;
					case 'make_unread': $trans_stat = $this->admin_communication_model->change_admin_message_status('0',$type); break;
	...
	public function conversation($type=NULL,$id=NULL,$userid=NULL)
	{
		$data = NULL;
		$data['current_menu'] = $type;
		$data['admin_permissions'] = $this->admin_permissions;
		if(array_key_exists('admin-communication', $this->admin_permissions)):
			$data['conversations'] = $this->admin_communication_model->get_conversation_by_id($type,$id,$userid);
			...
			if($type=='inbox' && $data['conversations']->ismsgseen==0){

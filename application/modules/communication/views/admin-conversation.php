        ...
        <ul id="conversation">
          ...
          <li>
            <div class="accordionButton <?php //echo ($open_msg_detail)?'on':'';?>"> <?php echo htmlentities($conversations->subject); ?> <em>
              ...
              </em> <em><?php echo $this->general->long_date_time_format($conversations->messagedate); ?></em> </div>
            <div class="accordionContent" <?php //echo ($open_msg_detail)?'style="display: block;"':'style="display: none;"';?>>
              <ol class="mailadd left">
                <?php $userdetail=$this->general->fetch_members_selected_fields(array('username as name','email'),array('id'=>$userid));?>
                <li>
                	<?php 
						if($current_menu=='inbox'){
							echo $userdetail->name.'<em>('.$userdetail->email.')</em></br>';
							echo "To Admin";
                ...
                <div class="attachmentDownloadContainer">
                  <?php if($conversations_attach->id != NULL && $conversations_attach->file_name != NULL){?>
                  	...
                  	<?php
						...
						echo anchor(ADMIN_DASHBOARD_PATH.'/communication/attachment/'.$current_menu."/".$conversations_attach->msg_id.'/'.$conversations_attach->id.'/'.$userid, $conversations_attach->file_name. ' ('.ceil($conversations_attach->file_size).'Kb)' ,array('class' => 'msg_attachment', 'title' => ''));
				    	}
					?>
                </div>
                <?php } ?>
                 <div class="clearfix"></div>
                  <li>
                   <a href="<?php echo site_url(ADMIN_DASHBOARD_PATH.'/communication/compose/'.$userid) ?>" class="butn" style="float:right">Reply </a>
                   </li>
              </section>
            </div>
          </li>

...
class Admin_dispute_model extends CI_Model 

{
	...
	public function count_total_disputes($status)
	{
		$this->db->select('R.*');
		$this->db->from('reportuser R');
		$this->db->join('members M', 'R.reporterid = M.id', 'left');
		$this->db->join('members M2', 'R.offenderid = M.id', 'left');
		...
		if($this->input->post('srch')!=""){
			...
			$this->db->where("(M.username LIKE '%".$this->input->post('srch',TRUE)."%' OR M2.username LIKE '%".$this->input->post('srch',TRUE)."%' OR M.email LIKE '%".$this->input->post('srch',TRUE)."%' OR M.email LIKE '%".$this->input->post('srch',TRUE)."%')");
	...
	public function get_dispute_lists($status, $limit, $start)
	{

		$this->db->select('R.*,R.id as reportid,Md.name as reporter,Md2.name as offender,R.offenderid,R.reporterid,R.bid_id');
		$this->db->from('reportuser R');
		$this->db->join('members M', 'R.reporterid = M.id');
		$this->db->join('members M2', 'R.offenderid = M2.id');
		$this->db->join('members_details Md', 'M.id = Md.user_id','left');
		$this->db->join('members_details Md2', 'M2.id = Md2.user_id','left');

		if($status && $status != 'all')
			$this->db->where('R.remarks', $status);

		if($this->input->post('srch')!=""){

			$this->db->where("(M.username LIKE '%".$this->input->post('srch',TRUE)."%' OR M2.username LIKE '%".$this->input->post('srch',TRUE)."%' OR M.email LIKE '%".$this->input->post('srch',TRUE)."%' OR M2.email LIKE '%".$this->input->post('srch',TRUE)."%')");
	...
	public function get_all_conversation($bidid)
	{
		$this->db->select('c.*');
		$this->db->from('communication c');
		$this->db->where('c.bid_id',$bidid);
	...
	public function get_reporter_offender($reportid)
	{

		
		$this->db->select('r1.reporterid ,r2.offenderid,m.*');
		$this->db->from('members_details m');
		$this->db->join('reportuser r1',"m.user_id=r1.reporterid and r1.id=$reportid");
		$this->db->join('reportuser r2',"m.user_id=r2.offenderid and r2.id=$reportid");

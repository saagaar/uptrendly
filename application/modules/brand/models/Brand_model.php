...
class Brand_model extends CI_Model
{
	...
	{	
		...
		$ceiling_likes=$this->input->post('filterceilinglikes',true);
		$profession=$this->input->post('filterprofession',true);
		$agerange=$this->input->post('filterage',true);
		$gender=$this->input->post('filtergender',true);
		$name=$this->input->post('filtername',true);
		$category_id=$this->input->post('filtercategory',true);
		$price=$this->input->post('filterpricerange',true);
		$cond='';
		if($audience_country)
		{
		...
		if($ceiling_likes)
		{
			$likes=explode('_',$ceiling_likes);
			if(count($likes)>1)
			$cond=$cond." and d.ceiling_likes between $likes[0] and $likes[1]";
			else 
				$cond=$cond." and d.ceiling_likes > $likes[0]";
		}
		if($name)
		{
			$cond=$cond." and d.name like('%$name%')";
		}
		if($profession)
		{
			// $ocond=$cond." and d.profession='$profession'";
		}
		if($agerange)
		{
			$age=explode('_',$agerange);
			if(count($age)>1)
			$cond=$cond." and d.age between $age[0] and $age[1]";
			else 
				$cond=$cond." and d.ceiling_likes > $age[0]";
		}
		if($gender)
		{
			$cond=$cond." and d.gender='$gender'";
		}
		if($category_id)
		{
			$cond=$cond." and d.category_id='$category_id'";
		}
		if($price)
		{
			$indprice=explode('_',$price);
			if(count($indprice)>1)
			$cond=$cond." and m.price between $indprice[0] and $indprice[1]";
			else 
				$cond=$cond." and m.price > $indprice[0]";
		}
		$join='';
		...
		$sql=$this->db->query(
								...
								SELECT sum(total_reach) FROM '.$prefix.'member_socialmedia where user_id=m.id
								
								) AS total_media_reach,
								(
										
										SELECT GROUP_CONCAT(profession SEPARATOR ",") FROM '.$prefix.'member_profesion  mp
										JOIN '.$prefix.'profession p ON (p.id=mp.profession_id) WHERE mp.user_id=m.id
										GROUP BY mp.user_id 
									) AS profession,
								(select country_code from '.$prefix.'audience_geography where user_id=m.id order by number_user desc limit 1 ) as audience_country,
									d.name AS member_name,d.cover_image,d.age,d.gender,d.ceiling_likes,m.available_status,d.country as user_country,s.*,m.price
								...
								WHERE user_type="4" AND m.STATUS="1"'. $cond
							 );
	  $this->db->last_query();
	...
	public function getuserproduct($userid=false,$proposaltype=false)
	{	
		...
		$query=$this->db->query('
									...
									left JOIN '.$prefix.'pricerange pr ON (p.price_range=pr.id)'.
									$join.'
									WHERE p.create_type="campaign" '.$cond.' order by p.id desc
								');
		...
		$query=$this->db->query('
								SELECT
									...
									(
										...
										SELECT GROUP_CONCAT(profession SEPARATOR ",") FROM '.$prefix.'member_profesion  mp
										JOIN '.$prefix.'profession p ON (p.id=mp.profession_id) WHERE mp.user_id=m.user_id
										GROUP BY ms.user_id 
									) AS profession,
									(
									...
									b.*,b.id as bid_id,b.status as bid_status,ms.*,dp.id as draft_id,dp.draft_accept,p.product_code,m.name as member_name,m.country ,m.ceiling_likes,m.age,m.phone,m.gender,p.name as product_name,dp.description ,dp.link,dp.image
									...
									left join '.$prefix.'member_socialmedia ms on (b.mediaid=ms.media_type_id and ms.user_id=m.user_id)
									left join '.$prefix.'socialmedia_settings ss on (ms.media_type_id=ss.id)
	...
	public function inviteuser()
	{
		...
		$sendername=$this->session->userdata(SESSION.'fullname');
		$senderemail=$this->session->userdata(SESSION.'email');
		...
		$this->notification->send_email_notification($template_code, '', $from, $to, '', '', $parseElement, $newimagearr);
	...
	public function get_product_report($productid)
	{
		$this->db->select('*');
		$this->db->from('products p');
		$this->db->join('product_bids pb','p.id=pb.product_id');
		$this->db->join('final_uploaded_content uc','uc.bid_id=pb.id','left');
		$this->db->join('report r','uc.id=r.content_id and r.bid_id=pb.id','left');
		$this->db->where(array('p.id'=>$productid,'p.status'=>'4'));
		$query=$this->db->get();
		if($query->num_rows()>0)
		{
			return $query->result();
	...
	public function get_objective_by_product($productid)
	{
		$this->db->select('*');
		$this->db->from('product_objective p');
		$this->db->join('objective o','p.objective_id=o.id');

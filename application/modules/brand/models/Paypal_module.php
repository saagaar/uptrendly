...
class Paypal_module extends CI_Model 
{
	...
	public function set_paypal_form_submit()
	{
		...
		$getuser=$this->general->get_user_details($this->user_id);
		$taxpercent=$this->general->get_tax_percent_by_country($getuser->country);
		...
		// 	$datapackage=$this->account_module->get_membership_package_byid($this->packageid);
			...
			$this->tax=($taxpercent/100)*$this->amount;
			$this->item_name = 'Top up Balance:'.' '.$this->amount;
		// 	//insert transaction record to transaction table
			$this->invoice_id = $this->insert_topup_transaction();
		...
		$this->paypal_class->add_field('return', site_url(BRAND.'paypal_success')); 		
		$this->paypal_class->add_field('cancel_return', site_url(BRAND.'paypal_cancel')); 		
		...
		$this->paypal_class->add_field('tax', $this->tax);
	...
	public function insert_topup_transaction()
	{
		$data = array(
		   	'user_id'            => $this->user_id,
			'amount'             => $this->amount,

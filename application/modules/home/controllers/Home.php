...
class Home extends CI_Controller
{
	...
	public function index()
	{
		...
		$this->data['account_menu_active']='home';
		$this->template
			->set_layout('home')
			->enable_parser(FALSE)
	...
	{
			$this->data['brands']=count($this->general->get_data('members',array('user_type'=>3)));
			$this->data['account_menu_active'] = "brands";
			$this->page_title = WEBSITE_NAME;
			$this->template
			->set_layout('home')
			->enable_parser(FALSE)
			...
			->title($this->page_title)
			->build('v_brand', $this->data);
	}
	
	public function demo()
	{
			$this->data['brands']=count($this->general->get_data('members',array('user_type'=>3)));
			$this->data['account_menu_active'] = "demo";
			$this->page_title = WEBSITE_NAME;
			$this->template
			->set_layout('home')
			->enable_parser(FALSE)
			...
			->title($this->page_title)
			->build('v_demo', $this->data);
	}
	public function contact()
	{
			$this->data['brands']=count($this->general->get_data('members',array('user_type'=>3)));
			$this->data['account_menu_active'] = "contact";
			$this->page_title = WEBSITE_NAME;
			$this->template
			->set_layout('home')
			->enable_parser(FALSE)
			...
			->title($this->page_title)
			->build('v_contact', $this->data);
	}

	public function creator()
	{
		$this->data['creators']=count($this->general->get_data('members',array('user_type'=>4)));
		$this->page_title = WEBSITE_NAME;
		$this->data['account_menu_active'] = "creators";
			$this->template
			->set_layout('home')
			->enable_parser(FALSE)
			->title($this->page_title)
			->build('v_creator', $this->data);
	}
	...
	public function contact_us()
	{
		...
		$template='contact';
		$from='advertiser@uptrendly.com';
		$to='gaurav.bista@mns.com.np';
		$parse_element=array(
								'message'		=>	$message,
								'name'			=> 	$name,
								'contact'		=>	$contactno,
		...
		$this->notification->send_email_notification($template, '', $from, $to, '', '', $parse_element, array());
		$this->db->insert('emts_contact',$parse_element);

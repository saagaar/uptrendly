...
class Brand_model extends CI_Model
{
	...
	{	
		...
		if($profession)
		{
			$cond=$cond." and d.profession='$profession'";
		}
		if($agerange)
		{
		...
		$sql=$this->db->query(
								...
								(select country_code from '.$prefix.'audience_geography where user_id=m.id order by number_user desc limit 1 ) as audience_country,
									d.name AS member_name,d.cover_image,p.profession,d.age,d.gender,d.ceiling_likes,m.available_status,d.country as user_country,s.*,m.price
								FROM '.$prefix.'members m  
								left JOIN '.$prefix.'members_details d ON m.id=d.user_id
								left JOIN '.$prefix.'profession p ON p.id=d.profession
	...
	{	
		...
		$query=$this->db->query('
									...
									$join.'
									WHERE p.create_type="'.$type.'" '.$cond.' order by p.id desc
								');
	...
	public function getproposal($productcode)
	{
		...
		$query=$this->db->query('
								SELECT
									...
									b.*,b.id as bid_id,b.status as bid_status,ms.*,dp.id as draft_id,dp.draft_accept,p.product_code,m.name as member_name,m.country ,m.ceiling_likes,m.age,m.phone,m.gender,p.name as product_name,dp.description ,dp.link,po.profession,dp.image
									...
									left join '.$prefix.'profession	 po on (po.id=m.profession)

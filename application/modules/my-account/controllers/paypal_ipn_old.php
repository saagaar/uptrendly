					...
					if(strtolower(trim($this->business))==strtolower(trim($this->paypal_business_email)))
					{
						...
						if($this->get_txn_data)
						{
							...
							if($this->get_txn_data->amount == $this->mc_gross)
							{
								...
								if($this->get_txn_data->transaction_type == 'pay_for_buy_now_product')
								{	
									//test email
									//$this->send_test_email('Pay For buy product; product id: ='.$this->get_txn_data->product_id."; user_id: ".$this->get_txn_data->user_id." ; invoice_id= ".$_REQUEST['invoice'], $post_string);			
									
									//update transaction records
									$this->paypal_module->paypal_transaction_data_update();
									
									//update user sales ordered payment status $auc_id, $user_id
									$this->paypal_module->update_sales_payment_status($this->get_txn_data->invoice_id,$this->get_txn_data->user_id);
									//update product table 
									$this->paypal_module->update_product_table_order_quantity($this->get_txn_data->product_id);
									
									//now send email to user to inform that his/her payment is completed
									$this->account_module->send_payment_status_email_to_buyer($this->get_txn_data->user_id, $this->get_txn_data->product_id, $this->invoice, $this->get_txn_data->amount, 'Paypal');
									
									//send email to admin to inform that a product has been sold.
									$this->account_module->product_sold_notification_admin($this->get_txn_data->user_id, $this->get_txn_data->product_id, $this->invoice, $this->get_txn_data->amount, 'Paypal');
									
									//send test email
									//$this->send_test_email('email parameters for seller ', "user id: ".$this->get_txn_data->user_id." order_id: ".$this->get_txn_data->order_id.", invoice: ".$this->invoice.", payment type:Paypal");
									
									//send email to all the sellers to inform that their product has been sold .
									$this->account_module->product_sold_notification_seller($this->get_txn_data->user_id, $this->get_txn_data->product_id, $this->invoice, $this->get_txn_data->amount, 'Paypal');
								
								}
								elseif($this->get_txn_data->transaction_type == 'pay_for_won_auction')
								{	
									//test email
									//$this->send_test_email('Pay For won auction; order_id: ='.$this->get_txn_data->order_id."; user_id: ".$this->get_txn_data->user_id." ; invoice_id= ".$_REQUEST['invoice'], $post_string);			
									
									//update transaction records
									$this->paypal_module->paypal_transaction_data_update();
									
									//update user product order table for payment status
									$this->paypal_module->update_product_order_for_won_auction($this->payment_status, $this->get_txn_data->order_id, $this->get_txn_data->user_id);
									
																
									//now send email to user to inform that his/her payment is completed
									//$this->account_module->paid_for_auction_notification_winner($this->get_txn_data->user_id, $this->get_txn_data->product_id, $this->invoice, $this->get_txn_data->amount, 'Paypal');
									
										
									//send email to admin to inform that a product has been sold.
									//$this->account_module->paid_for_auction_notification_admin($this->get_txn_data->user_id, $this->get_txn_data->product_id, $this->invoice, $this->get_txn_data->amount, 'Paypal');
																		
									
									//send test email
									//$this->send_test_email('email parameters for seller ', "user id: ".$this->get_txn_data->user_id." product_id: ".$this->get_txn_data->product_id.", invoice: ".$this->invoice.", payment type:Paypal");
									
									//send email to all the sellers to inform that their product has been sold .
									//$this->account_module->paid_for_auction_notification_seller($this->get_txn_data->user_id, $this->get_txn_data->product_id, $this->invoice, $this->get_txn_data->amount, 'Paypal');

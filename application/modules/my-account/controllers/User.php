...
class User extends CI_Controller 
{
	...
	public function settings($type='profile')
	{
			...
			$this->data['allmediaactive']=$this->general->get_socialmedia_channel();
			...
			if($user_type=='4')
			{
				  ...
				  $this->data['profession']=$this->general->get_data('member_profesion',array('user_id'=>$user_id));
	...
	public function get_socialmedia_profile($userid)
	{
		$socialmedia=$this->general->get_socialmedia_channel();
		$newdata=array();
		foreach ($socialmedia as $key => $eachsocialmedia) 
		{
			$smedia=$this->general->get_data('socialmedia_profile',array('user_id'=>$userid,'media_id'=>$eachsocialmedia->id));
			$dataarr=array();
			foreach($smedia as $newsmedia)
			{
				$dataarr[]=$newsmedia->url;
			}
			$newdata[$eachsocialmedia->id]=$dataarr;
		}
	...
	public function view_table_data($table,$column=false,$value=false,$select='*')
	{
		echo '<pre>';
	...
	{	
		   ...
		   if($this->session->userdata(SESSION.'usertype')=='3')
			...
			$user_type = $this->session->userdata(SESSION . 'usertype');
      		...
      		$this->data['active_products']=$this->general->get_product_by_user($user_id);
			  $this->page_title = 'Brand Dashboard' . ' - ' . WEBSITE_NAME;
	...
	public function Detailmessages($productid=false)
	{
			try
			{
				// if(($productid!='0') && (!$productid))  throw new Exception("Error Processing Request", 1);
			   ...
			   if($this->session->userdata(SESSION.'usertype')=='3')
		        {
					...
					$this->data['account_title']="brand";
					$user_type = $this->session->userdata(SESSION . 'usertype');
		      		$messagedetail=$this->account_module->getdetailmessage($productid);
		      		$this->account_module->makemessageseen($productid);
				...
				$this->session->set_flashdata('error_message','No chat history Found');
				redirect(site_url(MY_ACCOUNT.'messages/inbox'));
			}
	...
	public function getdetailmessage($productid)
	{
		try
		{
				...
				$memberdetail=$this->general->get_single_row('members_details',array('user_id'=>$user_id));
			 	$avatar=$this->general->get_profile_image($memberdetail->cover_image);
				$value['data']=$this->account_module->getdetailmessage($productid);
	...
	public function gettrackid()
	{
		...
		{
				$data=$this->general->get_data('final_uploaded_content',array('bid_id'=>$bidid));
				foreach ($data as $key => $value) 
				{
					$newarray[$value->uploaded_media]=$value->link;
				}
				...
				{
					echo json_encode(array('success_message'=>'Something Went wrong,Please try again in a while'));
				}
		}
	...
	public function savetrackid()
	{
		...
		if((count($check))<1)
		{
			...
			$uploadedcontent[]=array('bid_id'=>$bidid,'link'=>$fb_page,'uploaded_media'=>'fb_page','status'=>'1');
			if(isset($fb_profile) && trim($fb_profile)!='')
			$uploadedcontent[]=array('bid_id'=>$bidid,'link'=>$fb_profile,'uploaded_media'=>'fb_profile','status'=>'1');
			if(isset($instagram) && trim($instagram)!='')
			$uploadedcontent[]=array('bid_id'=>$bidid,'link'=>$instagram,'uploaded_media'=>'instagram','status'=>'1');
			$this->db->delete('final_uploaded_content',array('bid_id'=>$bidid));
			 $id=$this->db->insert_batch('final_uploaded_content',$uploadedcontent);	
			

		if($id)
		{
			$this->general->update_data('product_bids',array('upload_status'=>'1','upload_date'=>date('Y-m-d')),array('id'=>$bidid));
	...
	public function download_report()
    {
    	...
    	$reports=$this->account_module->get_all_reports();
    	$this->data['reports']=$reports;
    	$view=$this->load->view('ajax_report_pdf',$this->data,true);
						...
						$productbidnew=$this->general->get_single_row('product_bids',array('id'=>$bidid));
							if($productbid->status=='0' && $productbidnew->status=='1')
							{
								$parseElement=array(
												...
												'LINK'				=>		'<a href="'.site_url(BRAND.'getproposalbyproduct/'.$productdetail->product_code).'">'.site_url(BRAND.'getproposalbyproduct/'.$productdetail->product_code).'</a>',
							...
							if($productbid->status=='0' && $productbidnew->status=='4')
							{
								...
								$template_code='product_rejected';
								$from=SYSTEM_EMAIL;
								$to=$memberpropsender->email;
								$this->notification->send_email_notification($template_code, '', $from, $to, '', '', $parseElement, array());
								$this->general->savethisasnotification($productdetail->brand_id,$notifymsg,$productid);

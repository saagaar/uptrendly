...
class Brand_model extends CI_Model
{
	...
	{	
		...
		$sql=$this->db->query(
								...
								JOIN '.$prefix.'members_details d ON m.id=d.user_id
								JOIN '.$prefix.'member_socialmedia s ON (m.primary_media=s.media_type_id)
	...
	{	
		...
		$query=$this->db->query('
									SELECT 
										...
										p.name AS product_name,product_code,p.id as product_id,p.status,p.submission_deadline,pr.price_range,p.description,p.product_url
		...
		$query=$this->db->query('
								SELECT
									...
									(
										...
										where dp.bid_id=b.id and dp.bid_status=2 and dp.draft_accept="0"  limit 1
									) as draft,
									(
										SELECT count(pb.id) from  '.$prefix.'product_bids pb 
										'.$join.'
									...
									b.*,b.id as bid_id,ms.*,dp.id as draft_id,p.product_code,m.name as member_name,p.name as product_name,dp.description ,dp.link
										
									FROM '.$prefix.'product_bids b
									join '.$prefix.'products p on (p.id=b.product_id)
									left join '.$prefix.'draft_promotion dp on (b.id=dp.bid_id and dp.bid_status=2)
	...
	public function acceptreject_draft()
	{
		...
		$this->db->select('p.id,pb.user_id,p.name');
		...
		if((count($data))>0) {
			...
			if($id){
				if($info==1){
					...
					$notifymsg='Your draft of promotion for the campaign '.$productname.' has been accepted.Please check email';
					$this->general->savethisasnotification($user,$notifymsg);
					return array('success_message'=>'Draft has been Accepted');
				}			
				else 
				{
					...
					$this->general->savethisasnotification($user,$notifymsg);
					return  array('success_message'=>'Draft has been Rejected');
				} 
			}
			else {
			...
			return array('error_message'=>'You are not authroized to perform the operation');
		}
	}

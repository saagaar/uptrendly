	...
	public function getproposal($productcode)
	{
		...
		$query=$this->db->query('
								SELECT
									...
									b.*,b.id as bid_id,ms.*,dp.id as draft_id,dp.draft_accept,p.product_code,m.name as member_name,p.name as product_name,dp.description ,dp.link
										
									FROM '.$prefix.'product_bids b
									join '.$prefix.'products p on (p.id=b.product_id)
									left join '.$prefix.'draft_promotion dp on (b.id=dp.bid_id  and dp.draft_accept="1")

...
/*******************For changing Availability option****************/
	...
	public function download_partial_report($bidid)
    {
    	...
    	$view=$this->load->view('v_report_portion',$this->data,true);
    	$this->pdf->downloadpdf('',$view,'D');
	...
	public function getproposalbyproduct($productcode,$id=false)
	{
			...
			// }else
			// {
					$proposals=$this->model->getproducts($productcode);
	...
	public function send_draft()
	{
		...
		if($this->form_validation->run()==TRUE)
		{
				...
				if($res)
				{
					if(isset($res['error']))
					{
						echo json_encode(array('error_message'=>$res['error']));
					}
					else
					{
				...
				{
					 echo json_encode(array('error_message'=>'No data updated'));
				}
	...
	public function uptrendly_brand()
	{
		$this->data['brands']=$this->model->get_brands();
				  $this->data['user_type'] = "creator";	
				  $this->page_title = 'Creator Dashboard' . ' - ' . WEBSITE_NAME;
				  $this->data['account_menu_active'] = "uptrendly_brand";
	     		  $this->data['account_title'] = 'Creators';
				  // $this->data['proposals']=$proposals;
				  // $this->data['productname']=$check->name;
				  // $this->data['product_id']=$check->id;
				...
				->enable_parser(FALSE)
				->title($this->page_title)
				->build('v_uptrendly_brand', $this->data);
	}

...
class Creator_model extends CI_Model
{
	...
	public function inviteuser()
	{
		...
		$creatorname=$this->session->userdata(SESSION.'fullname');
		$creatoremail=$this->session->userdata(SESSION.'email');
		
		if($_FILES)
		{

			    $fdata=$this->multiple_image_upload('uploadimage',INVITATION_IMAGE_PATH,'encrypt');
			  	if(isset($fdata['error'])){ return (array('error_message'=>$fdata['error']));exit;}
			 	$count=	count($fdata);
				if($count>0)
				{
					...
					foreach ($fdata as $key => $value) 
					{
						...
						$imageattach->path=INVITATION_IMAGE_PATH;
						$imageattach->name=$images;
		...
		$parseElement = array
					(
						...
						"MESSAGE"			=> $message,
						"INFLUENCER_EMAIL"	=>	$creatoremail,
						"INFLUENCER_NAME"	=>	$creatorname,
						"SENDER_EMAIL"		=> $this->session->userdata(SESSION.'email'),
						"SENDER_NAME"		=> $this->session->userdata(SESSION.'fullname')
					);
		...
		$this->notification->send_email_notification($template_code, '', $from, $email, '', '', $parseElement, $newimagearr);
		$notifymsg='You have Been invited by Brand '.$userdata->username.' .Check your email';
	...
	{	
		...
		$query=$this->db->query('
									SELECT 
										...
										p.name AS campaign_name,product_code,pb.user_id,p.id as product_id,pb.status,p.submission_deadline,p.post_date,p.description,p.product_url,p.product_name,c.name as category_name,pb.status as bid_status,pb.id as bid_id,dp.draft_accept,pb.upload_status
									FROM '.$prefix.'products p 
									join '.$prefix.'product_bids pb on p.id=pb.product_id
	...
	public function send_draft_promotion()
	{
		...
		else
		{
			...
			{
					$this->db->select('email,m.username,p.name,m.id,p.product_code');
					$this->db->from('products p');
							...
							if($_FILES['uploadimage']['name'])
								{
									...
									{
										$data['image']	= $fdata['file_name'];
										$imageattach->path=DRAFT_IMAGE_PATH;
										$imageattach->name=$fdata['file_name'];
									}
									else
									{
										return array('error'=>$this->error_img);

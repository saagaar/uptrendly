...
//validation for add/edit product in inventory
	public $validate_campaign_creation =  array(	
		...
		array('field' => 'product_url', 'label' => 'Product URL', 'rules' => 'trim|required|max_length[100]'),
		array('field' => 'owner_name', 'label' => 'Full Name', 'rules' => 'trim|required|max_length[50]|min_length[2]'),
		array('field' => 'vatno', 'label' => 'Vat/Pan', 'rules' => 'trim|required|max_length[50]|min_length[2]'),
...
//validation rules for change password
	...
	public $validate_general_brand=array(
		...
		array('field' => 'phone', 'label' => 'Phone', 'rules' => 'trim|required|max_length[15]'),
		array('field' => 'email', 'label' => 'Email', 'rules' => 'trim|required|valid_email'),
		array('field' => 'pan_no', 'label' => 'Pan/Vat', 'rules' => 'trim|required|max_length[20]'),
		...
		if(isset($_FILES) && count($_FILES)>0 && $_FILES['uploadimage'])
		{
			...
			if(is_array($creatorselected) && (count($creatorselected)>0))
			{
				...
				$this->db->insert_batch('product_bids', $creatorlist);
					...
					$this->db->where_in('id',$creatorselected);
					$query=$this->db->get();
					$emaillinear='';
					$emaillist=$query->result('array');
					foreach ($emaillist as  $value) 
					{
						$emaillinear=$value['email'].','.$emaillinear;
					}
			}
	...
	public function send_message()
	{
				...
				// {
					...
					// 	$sender_id=$current_user;
					// }
					// elseif($current_user==$productbids->user_id)
					// {
							$sender_id=$current_user;
							$receiver_id=1;
					// }else
				...
				$member=$this->general->get_single_row('members_details',array('user_id'=>$current_user));		
	...
	public function getmessage($type='inbox')
	{
			...
			$this->db->select('bid_id,max(c.id) as pid');
			...
			if($filt) $this->db->where($filt);
			 ...
			 $this->db->select('m.email as sender_email,ec.bid_id,n.email as receiver_email,ec.message,ec.ismsgseen,ec.sender_id,ec.receiver_id');
	...
	public function getdetailmessage($bidid)
	{
		...
		$cond='1=1';	       
			...
			$this->db->select('m.name as sender_name,c.messagedate,m.cover_image as sender_image,n.cover_image as receiver_image,c.bid_id,n.name as receiver_name,c.message,ismsgseen,c.sender_id,c.receiver_id');
			$this->db->from('communication c');
			$this->db->join('communication_attachment ca','c.id=ca.msg_id','left');
			$this->db->join('members_details m','c.sender_id=m.user_id');
			$this->db->join('members_details n','c.receiver_id=n.user_id');
				 $this->db ->or_group_start();
			     $this->db->where(array('c.receiver_id'=>$userid));
				 $this->db->or_where(array('c.sender_id'=>$userid));
				 $this->db->group_end();
...
/* sending email to supplier for matched category if auction type is public	 */
	...
	public function get_creator_details($userid){
		try{
			...
			$this->db->select('m.*,md.*,p.profession as  profession_name');
			$this->db->from('members m');
			$this->db->join('members_details md','m.id= md.user_id','left');
